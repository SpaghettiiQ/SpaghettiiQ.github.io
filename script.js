...
textInput.addEventListener('keydown', (e) => {
	if ((textInput.textContent.length + 1 > textLimit)) {
		if (String.fromCharCode(e.keyCode).match(/(\w|\s)/g) && !e.ctrlKey && !e.metaKey) {
			e.preventDefault();
		}
	}
	if (e.key === "Enter") {
...
const calContainerElem = document.getElementsByClassName('calContainer')[0];

const calBtn = document.getElementById('calBtn');
var calBtnHold = false;

calBtn.addEventListener('mousedown', function(e) {
	e.preventDefault();
	calBtn.classList.add('pop');
	calBtnHold = true;
});
...
function toggleCal() {
	if (calContainerElem != null) {
		if (calBtn.classList.contains('active')) {
			calContainerElem.classList.add('active');
		} else {
			calContainerElem.classList.remove('active');
		}
	}
}
...
window.addEventListener('click', function(e) {
	...
	while (target && target !== document.documentElement) {
		...
		for (let className of classNames) {
			if (className.includes('cal')) {
				foundCal = true;
				break;
			}
		}
		if (target.id.includes('cal')) {
			foundCal = true;
			break;
		}
